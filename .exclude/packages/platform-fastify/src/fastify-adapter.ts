...
import { FastifyStaticAssets } from './fastify-static-assets.interface';
import { PointOfViewOptions } from './point-of-view-options.interface';
import { FastifyServerOptions } from './fastify-server-options.type';
...
export class FastifyAdapter implements HttpServer {
  ...
  async useStaticAssets(options: FastifyStaticAssets) {
    const fastifyStatic = await Utils.loadPackage(
      'fastify-static',
      'FastifyAdapter#useStaticAssets',
    );

    return this.register(fastifyStatic, options);
  }

  async setViewEngine(options?: PointOfViewOptions) {
    const pointOfView = await Utils.loadPackage(
      'point-of-view',
      'FastifyAdapter#setViewEngine',
    );

    return this.register(pointOfView, options);
  }
  ...
  async registerParserMiddleware() {
    ...
    return this.instance.register(fastifyFormBody);
  }
